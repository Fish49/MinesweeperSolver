...
def onSubmit():
    ...
    difficulty = difficultyEntry.get()
    playRisky = playRiskyToggle.get() #determines whether it will guess at the beginning
...
difficulties=['GoogleEasy', 'GoogleMedium', 'GoogleHard']
difficultyEntry = tk.StringVar(window)
difficultyDisclaimer = tk.Label(window, text='Difficulty:')
difficultyDropdown = tk.OptionMenu(window, difficultyEntry, *difficulties)
playRiskyToggle = tk.BooleanVar(window)
playRiskyToggleBox = tk.Checkbutton(window, variable=playRiskyToggle, text='Play Risky')
submitButton = tk.Button(window, text='Submit', command=onSubmit)

difficultyDisclaimer.pack()
difficultyDropdown.pack()
playRiskyToggleBox.pack()
submitButton.pack()
